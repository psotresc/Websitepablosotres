 ...
 * Add breadcrumb.
 *
 * @action nepalbuzz_after_header
 *
 * @since NepalBuzz 0.1
 */
if( !function_exists( 'nepalbuzz_add_breadcrumb' ) ) :

	function nepalbuzz_add_breadcrumb() {
		...
		if( $options['breadcrumb_option'] ){
			...
			echo nepalbuzz_custom_breadcrumbs( $show_on_home, $delimiter );
		}
		else
			return false;
	}
...
add_action( 'nepalbuzz_before_content', 'nepalbuzz_add_breadcrumb', 60 );
 ...
 * @since NepalBuzz 0.1
 */
if( !function_exists( 'nepalbuzz_custom_breadcrumbs' ) ) :

	function nepalbuzz_custom_breadcrumbs( $show_on_home, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'nepalbuzz' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'nepalbuzz' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'nepalbuzz' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'nepalbuzz' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'nepalbuzz' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'nepalbuzz' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'nepalbuzz' ), max( $paged, $page ) );
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end nepalbuzz_breadcrumb_lists
endif;
